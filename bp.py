...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  2 15:38:43 2018
...
# Forward function, updates self.Z, self.A
def fw(net, X, y):
    n = len(net)
...
# Calculates the pseudo-residual at each layer
...
def weight_gradient(I, dZ):
    dW = np.dot(np.transpose(I), dZ) / dZ.shape[0]
...
def fwbk(net, X_train, y_train, lr, momentum):
...
def train(net, X, y, batch_size, epochs, lr, momentum=0, decay=0, verbose=0):
    ...
    for e in xrange(epochs):
        ...
        for i in xrange(0, n, batch_size):
            ...
            loss, acc = fwbk(net, X_train, y_train, lr, momentum)
